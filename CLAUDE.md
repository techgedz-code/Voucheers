...
**Swappable games (presentation only).** Because the draw is server-authoritative, the on-screen game is purely cosmetic and the prize logic is fully decoupled from it. `campaigns.game_type` (enum `spin_wheel | football`, migration `0006`) picks which game the customer sees; the merchant chooses it via `GameTypeField` in campaign settings. **Prizes/voucher types and win weights are shared across all games — configured once.** `CustomerFlow.tsx` branches on `gameType`: `spin_wheel` animates `components/SpinWheel.tsx` to the returned `prizeIndex`; `football` plays `components/FootballGame.tsx` (a 30-second canvas penalty game adapted from a standalone HTML game) purely for fun, then reveals the **same server-decided prize on the result screen, independent of the score** (score is flavour only — never tie reward to skill). Both games call one shared `drawPrize()` in `CustomerFlow`. Adding a future game = add an enum value + a component branch; no draw/issuance changes.
